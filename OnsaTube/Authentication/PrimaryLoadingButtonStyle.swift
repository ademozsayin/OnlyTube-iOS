...
struct PlusButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        return IconButton(configuration: configuration, icon: UIImage(systemName: "plus.circle")!)
    }
}
...
//    }
    ...
    var backgroundColor: Color {
        if isEnabled {
            if configuration.isPressed {
                return theme.primaryBackgroundColor.opacity(0.4)
            } else {
                return theme.tintColor
            }
        } else {
            return theme.primaryBackgroundColor.opacity(0.9)
        }
    }
